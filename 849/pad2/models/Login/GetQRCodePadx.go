...
func GetQRCODEPadx(Data GetQRReq) models.ResponseResult2 {
	...
	Algorithm.IPadDeviceType = Algorithm.AndroidPadDeviceType
	Algorithm.IPadVersion = Algorithm.AndroidPadVersion
	aeskey := []byte(lib.RandSeq(16)) //获取随机密钥
	deviceid := Data.DeviceID
	devicelIdByte, _ := hex.DecodeString(deviceid)

	DeviceToken, err := IPadGetDeviceToken(deviceid, Algorithm.AndroidPadModel, Data.DeviceName, Algorithm.AndroidPadDeviceType, int32(Algorithm.AndroidPadVersionx), *httpclient, Data.Proxy, Algorithm.MmtlsShortHost)
	if err != nil {
	...
	req := &mm.GetLoginQRCodeRequest{
		BaseRequest: &mm.BaseRequest{
			...
			ClientVersion: proto.Int32(int32(Algorithm.AndroidPadVersionx)),
			DeviceType:    []byte(Algorithm.AndroidPadDeviceType),
