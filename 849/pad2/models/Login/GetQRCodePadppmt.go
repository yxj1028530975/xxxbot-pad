...
func GetQRCODEPadppmt(Data GetQRReq) models.ResponseResult2 {
	//初始化Mmtls
	...
	Algorithm.IPadDeviceType = "Android-pad-34"
	Algorithm.IPadVersion = Algorithm.CarVersion
	aeskey := []byte(lib.RandSeq(16)) //获取随机密钥
	deviceid := Data.DeviceID
	devicelIdByte, _ := hex.DecodeString(deviceid)

	DeviceToken, err := IPadGetDeviceToken(deviceid, Algorithm.AndroidPadModel, Data.DeviceName, Algorithm.IPadDeviceType, int32(Algorithm.AndroidPadVersion), *httpclient, Data.Proxy, Algorithm.MmtlsShortHost)
	if err != nil {
