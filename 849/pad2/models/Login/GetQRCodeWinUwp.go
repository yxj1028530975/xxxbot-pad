...
func GetQRCODEWinUwp(Data GetQRReq) models.ResponseResult2 {
	...
	Algorithm.IPadVersion = Algorithm.WinUwpVersion
	aeskey := []byte(lib.RandSeq(16)) //获取随机密钥
	deviceid := Data.DeviceID
	devicelIdByte, _ := hex.DecodeString(deviceid)

	DeviceToken, err := IPadGetDeviceToken(deviceid, Algorithm.WinModel, Data.DeviceName, Algorithm.WinDeviceType, int32(Algorithm.WinUwpVersion), *httpclient, Data.Proxy, Algorithm.MmtlsShortHost)
	if err != nil {
	...
	req := &mm.GetLoginQRCodeRequest{
		BaseRequest: &mm.BaseRequest{
			...
			ClientVersion: proto.Int32(int32(Algorithm.WinUwpVersion)),
	...
	if getloginQRRes.GetBaseResponse().GetRet() == 0 {
		...
		err := comm.CreateLoginData(comm.LoginData{
			...
			ClientVersion: Algorithm.WinUwpVersion,
			Cooike:        ph1.Cookies,
			Proxy:         Data.Proxy,
