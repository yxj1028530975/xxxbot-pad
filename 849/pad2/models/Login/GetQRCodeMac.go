...
func GetQRCODEMac(Data GetQRReq) models.ResponseResult2 {
	...
	Algorithm.IPadDeviceType = Algorithm.MacDeviceType
	Algorithm.IPadVersion = Algorithm.MacVersion
	aeskey := []byte(lib.RandSeq(16)) //获取随机密钥
	deviceid := Data.DeviceID
	devicelIdByte, _ := hex.DecodeString(deviceid)

	DeviceToken, err := IPadGetDeviceToken(deviceid, Algorithm.MacModel, Data.DeviceName, Algorithm.MacDeviceType, int32(Algorithm.MacVersion), *httpclient, Data.Proxy, Algorithm.MmtlsShortHost)
	if err != nil {
	...
	req := &mm.GetLoginQRCodeRequest{
		BaseRequest: &mm.BaseRequest{
			...
			ClientVersion: proto.Int32(int32(Algorithm.MacVersion)),
			DeviceType:    []byte(Algorithm.MacDeviceType),
