...
func GetQRCODEWinUnified(Data GetQRReq) models.ResponseResult2 {
	...
	Algorithm.IPadDeviceType = Algorithm.WinunifiedDeviceType
	Algorithm.IPadVersion = Algorithm.WinUnifiedVersion
	aeskey := []byte(lib.RandSeq(16)) //获取随机密钥
	deviceid := Data.DeviceID
	devicelIdByte, _ := hex.DecodeString(deviceid)

	DeviceToken, err := IPadGetDeviceToken(deviceid, Algorithm.WinModel, Data.DeviceName, Algorithm.WinunifiedDeviceType, int32(Algorithm.WinUnifiedVersion), *httpclient, Data.Proxy, Algorithm.MmtlsShortHost)
	if err != nil {
	...
	req := &mm.GetLoginQRCodeRequest{
		BaseRequest: &mm.BaseRequest{
			...
			ClientVersion: proto.Int32(int32(Algorithm.WinUnifiedVersion)),
			DeviceType:    []byte(Algorithm.WinunifiedDeviceType),
