...
func GetQRCODEx(Data GetQRReq) models.ResponseResult2 {
	...
	Algorithm.IPadDeviceType = "iPad iOS18.0.1"
	Algorithm.IPadVersion = 0x18003526

	aeskey := []byte(lib.RandSeq(16)) //获取随机密钥
	deviceid := Data.DeviceID
	devicelIdByte, _ := hex.DecodeString(deviceid)

	DeviceToken, err := IPadGetDeviceToken(deviceid, Algorithm.IPadModel, Data.DeviceName, Algorithm.IPadDeviceType, int32(Algorithm.IPadVersionx), *httpclient, Data.Proxy, Algorithm.MmtlsShortHost)
	if err != nil {
	...
	req := &mm.GetLoginQRCodeRequest{
		BaseRequest: &mm.BaseRequest{
			...
			ClientVersion: proto.Int32(int32(Algorithm.IPadVersionx)),
